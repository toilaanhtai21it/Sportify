...
const app = {
  playSongs: { 
    ...
    loadCurrentSong: function() {//In thông tin song
    ...
    handleTotalTime: function() {
      audio.onloadedmetadata = function() {
        ...
        const minutes = Math.floor(songDuration / 60)//Tính thời gian
    ...
    audioPlaying: function(color = '#b3b3b3') {
      ...
      audio.ontimeupdate = function() { //in ra thời gian đã chạy
    ...
    handlePlaybackSliderBar: function() {   
      ...
      playbackSlider.onclick = function(e) {e.stopPropagation()} //chống nổi bọt
    ...
    handlePlayOrPauseAudio: function() {
      ...
      audio.onpause = function() {
        ...
        playBtn.src = `./assets/images/now-playing/play.PNG`
        $('title').innerText = `Spotify - Made by Tai`
  ...
  handlePlaylists: {
    ...
    handleCurrentPlaylist: {
      ...
      handleMainViewBackground: {
        renderBackground: function() {  
          ...
          this.handleHeaderOpacity(currentPlaylistContent[0].headerColor) //lướt xuống đổi màu
    ...
    handleOwnPlaylists: {
      renderOwnPlaylists: function() {
        const ownPlaylists = app.handlePlaylists.playlists.filter(playlist => !playlist.tag.includes('liked songs') && playlist.owner == 'Tai')
          .map(playlist => `
      ...
      styleLeftSidebarPlaylist: function() { //đổi màu khi lướt vào
  ...
  handlePlaylistPage: { // load lại bài khi nhấn nút play
          ...
          <div id="on-open-playlist__header">
            ...
            <div id="on-open-playlist__header__title">
              ...
              <div id="on-open-playlist__header__title__playlist-info">
                ${thisPlaylistInDB.tag.includes('own playlist') ? '<img id="on-open-playlist__header__title__playlist-info__img" src="./assets/images/user/user-avatar.jpg"><span id="on-open-playlist__header__title__playlist-info__owner">' + thisPlaylistInDB.owner : '<span id="on-open-playlist__header__title__playlist-info__owner"><span>Made for</span> Tai'} <span>• ${thisPlaylistInDB.songs.length} songs</span></span>
                </div>
            </div>
          </div>
